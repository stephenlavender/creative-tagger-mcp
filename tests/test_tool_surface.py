...
class ToolSurfaceTest(unittest.TestCase):
    ...
    def test_public_competitor_examples_are_synthetic(self) -> None:
        ...
        self.assertIn("Everwell Labs (Demo)", readme)
        self.assertNotIn("Hims & Hers", readme)
        self.assertIn("must use obviously\nsynthetic brands", agent_instructions)
    ...
    def test_tool_copy_uses_current_taxonomy_dimension_count(self) -> None:
        ...
        self.assertIn("main` branch still carries `0.2.4` package\nmetadata", readme)
    ...
    def test_local_release_upload_requires_a_new_exact_version(self) -> None:
        readme = README.read_text()

        self.assertNotIn("python -m twine upload dist/*", readme)
        self.assertIn(
            "dist/creative_tagger_mcp-NEXT_VERSION-py3-none-any.whl", readme
        )
        self.assertIn("dist/creative_tagger_mcp-NEXT_VERSION.tar.gz", readme)
        self.assertIn("already-published `0.2.4`\nidentity", readme)
    ...
    def test_release_smoke_checks_server_version_and_packaged_readme(self) -> None:
        ...
        self.assertIn('"unreleased 47-tool, 10-prompt candidate"', source)
        self.assertIn('"Do not rebuild or republish it as `0.2.4`"', source)
    ...
    def test_performance_tools_describe_funnel_scores(self) -> None:
        ...
        self.assertIn("strategy breakdown", competitor_detail_desc)
        ...
        self.assertEqual(
            competitor_detail_schema["properties"]["scan_id"]["type"], "integer"
        )
    ...
    def test_strategy_tool_forwards_demographic_and_date_controls(self) -> None:
        ...
        self.assertIn('"get_competitor_scan_detail": _get_competitor_scan_detail', source)
